...
socket.on('join_room_response',function(payload){
	...
	var newHTML = '<p style="color:#d9ae12;"> <b style="color:#a5c051;">'+payload.username+'</b> just entered the room</p>';
	var newNode = $(newHTML);
	newNode.hide();
...
socket.on('player_disconnected',function(payload){
	...
	var newHTML = '<p style="color:#fb8e2c;"> <b style="color:#a5c051;">'+payload.username+' </b> has left the room</p>';
	var newNode = $(newHTML);
	newNode.hide();
	$('#messages').prepend(newNode);
	newNode.slideDown(1000);
...
socket.on('send_message_response',function(payload){
	...
	var newHTML = '<p style="color:#d5e4b1;"><b style="color:#68a834;">'+payload.username+' says:  </b>'+payload.message+'</p>';
	var newNode = $(newHTML);
	newNode.hide();
	$('#messages').prepend(newNode);
	newNode.slideDown(1000);	
...
function makeInviteButton(socket_id){
	var newHTML = '<button type=\'button\' class=\'btn btn-invite\'>Invite</button>';
	var newNode = $(newHTML);
		...
		for(column = 0; column < 8; column++){
			...
			if(old_board[row][column] != board[row][column]){
				if(old_board[row][column] == '?' && board[row][column] == ' '){
					...
					$('#'+row+'_'+column).html('<img src="assets/images/empty_white.gif" alt="white square"/>');
				}
				else if(old_board[row][column] == '?' && board[row][column] == 'b'){
					$('#'+row+'_'+column).html('<img src="assets/images/empty_black.gif" alt="black square"/>');
				}
				else if(old_board[row][column] == ' ' && board[row][column] == 'w'){
					$('#'+row+'_'+column).html('<img src="assets/images/empty_white.gif" alt="white square"/>');
				}
					...
					$('#'+row+'_'+column).html('<img src="assets/images/white_empty.gif" alt="empty square"/>');
				}
				else if(old_board[row][column] == 'b' && board[row][column] == ' '){
					$('#'+row+'_'+column).html('<img src="assets/images/black_empty.gif" alt="empty square"/>');
				}
				else if(old_board[row][column] == 'w' && board[row][column] == 'b'){
					$('#'+row+'_'+column).html('<img src="assets/images/white_black.gif" alt="black square"/>');
				}
				else if(old_board[row][column] == 'b' && board[row][column] == 'w'){
					$('#'+row+'_'+column).html('<img src="assets/images/black_white.gif" alt="white square"/>');
				}
				else{
...
socket.on('game_over',function(payload){
	...
	$('#game_over').html('<h4>Game Over</h4><h5>'+payload.who_won+' won!</h5>');
